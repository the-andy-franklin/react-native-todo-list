...
task_router.get("/", async (c) => {
	...
	if (!user.data) return c.json({ message: "Tasks not found" }, 400);
...
task_router.post("/", taskPostBodyValidatorMiddleware, async (c) => {
	const { username }: { username: string } = c.get("jwtPayload");
	const result = await Try(() => User.findOne({ username }).populate("tasks").exec());
	if (result.failure) return c.json({ message: result.error.message }, 500);
	if (!result.data) return c.json({ message: "Tasks not found" }, 400);
